...
HEALTH_CHECK["checks"] += [{"name": "celery", "handler": lambda: (True, "")}]

ACTIVE_APPS = ["business", "employees", "channel"]

SERVICES = {
    "customers": {"host": environment.customers_host, "key": environment.customers_key,},
    "catalog": {"host": environment.catalog_host, "key": environment.catalog_key,},
}

ACCESS_TOKEN_EXP_DELTA = timedelta(hours=environment.access_token_exp)
JWT["AUTHENTICATION_HANDLERS"] = {"JWT": JWTAuthHandler}
