...
async def get_product(request: Request, product_id: ObjectId, business=Depends(get_business)):
    ...
    return await broker.get_product(business_id=business.id, product_id=product_id, params=request.query_params,)
...
async def update_product(product_id: ObjectId, request: Request, business=Depends(get_business)):
    ...
    return await broker.update_product(
        business_id=business.id, product_id=product_id, params=request.query_params, json=await request.json(),
    )
...
async def create_product(request: Request, business=Depends(get_business)):
    ...
    return await broker.create_product(business_id=business.id, params=request.query_params, json=await request.json(),)
